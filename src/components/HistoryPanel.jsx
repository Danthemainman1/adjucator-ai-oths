...
const SessionCard = ({ session, onView, onDelete }) => {
  ...
  return (
    <div className="glass-card p-0 overflow-hidden group">
      ...
      {/* Expanded Content */}
      {isExpanded && (
        <div className="border-t border-slate-700/50 p-4 animate-in slide-in-from-top-2">
          {/* Metadata */}
          {session.metadata && (
            <div className="flex flex-wrap gap-2 mb-4">
              {session.metadata.side && (
                <span className="text-xs px-2 py-1 bg-slate-800 rounded-lg text-text-secondary">
                  {session.metadata.side}
                </span>
              )}
          ...
          {/* Result Preview */}
          <div className="bg-slate-900/50 rounded-lg p-4 max-h-[400px] overflow-y-auto custom-scrollbar">
          ...
          {session.type === 'extemp' && session.input?.question && (
            <div className="mt-4 p-4 bg-slate-800/50 rounded-lg border border-slate-700/50">
              <h4 className="text-xs font-medium text-text-muted mb-2">Generated Question</h4>
...
const HistoryPanel = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 text-text-secondary hover:text-white hover:bg-slate-800 rounded-lg transition-colors disabled:opacity-50"
          ...
          <button
            ...
            className="p-2 text-text-secondary hover:text-white hover:bg-slate-800 rounded-lg transition-colors"
      ...
      {/* Import Status */}
      {importStatus && (
        <div className={`p-3 rounded-lg flex items-center gap-2 text-sm animate-in fade-in ${
      ...
      {filteredHistory.length > 0 ? (
        ...
        <div className="glass-card flex flex-col items-center justify-center min-h-[40vh] text-center">
          {history.length === 0 ? (
            <>
              <HistoryIcon className="w-16 h-16 text-slate-700 mb-4" />
              <h3 className="text-xl font-bold text-white mb-2">No Sessions Yet</h3>
              <p className="text-text-secondary max-w-md">
                Your analysis sessions will appear here. Try judging a speech, evaluating a board, 
                or using the live coach to get started!
              </p>
            </>
          ) : (
            <>
      ...
      {showClearConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="glass-card w-full max-w-md space-y-4 animate-in zoom-in-95">
